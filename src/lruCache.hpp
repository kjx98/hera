/*
 * lruCache - a templated C++11 based LRU cache class that allows
 * specification of * key, value and optionally the map container
 * type (defaults to * std::unordered_map)
 * By using the std::unordered_map and a linked list of keys it allows O(1)
 * insert, delete * and * refresh operations.
 *
 * This is a header-only library and all you need is the lruCache.hpp file
 *
 *
